      ...
      <div class="mx-auto w-full px-6 py-4 sm:py-20 flex flex-col items-center gap-10 animate-fade-in">
        <div class="flex flex-col gap-6 w-full items-stretch text-left">
          <div class="flex items-center gap-3">
            <div
              class="inline-flex items-center gap-2 bg-transparent border border-forest-link rounded-circular px-4 py-1.5 text-forest-link text-body-sm font-medium tracking-tight">
              <span class="w-2 h-2 rounded-circular bg-forest-link animate-pulse" />
              Edicion abierta a la comunidad
            </div>
          </div>
          <h1
            class="font-pp-mori font-bold text-heading sm:text-display leading-[0.95] text-midnight-pine tracking-tight">
            Encuentra articulos con
            <br />
            <span class="text-forest-link">criterio editorial</span>
          </h1>
          <p class="font-inter text-ash-gray text-body leading-subheading tracking-subheading">
          ...
          <div class="relative w-full mt-2">
            ...
            <input id="hero-search" v-model="searchQuery" type="text" placeholder="Buscar articulos, temas, autores..."
              class="w-full bg-canvas-white border border-light-gray-border rounded-soft py-[16px] pl-12 pr-4 text-body-sm text-midnight-pine placeholder:text-ash-gray outline-none focus:border-forest-link transition-colors"
              @input="onSearch" />
            <kbd
          ...
          <div class="flex flex-wrap items-center gap-2 pt-2">
            <span class="text-body-sm text-ash-gray">Lecturas destacadas:</span>
            <span v-for="topic in quickTopics" :key="topic"
              class="bg-transparent text-ash-gray border border-steel-border text-body-sm px-3 py-0.5 rounded-circular font-inter">
              {{ topic }}
            </span>
          </div>
        </div>
    ...
    <main class="flex-1 max-w-5xl mx-auto w-full px-5 sm:px-6 py-10 flex flex-col gap-8">
      <div
        ...
        <div class="flex flex-col gap-2">
          <p class="font-pp-mori text-heading-sm text-midnight-pine">Portada de articulos</p>
          <p class="font-inter text-ash-gray text-body">
        ...
        <span class="font-inter text-ash-gray text-body-sm">Pagina {{ paginated.page }} de {{ paginated.totalPages
          }}</span>
      </div>
      ...
      <div v-if="postPublic.length > 0" class="flex flex-col gap-5 animate-fade-in">
        <RouterLink v-for="post in postPublic" :key="post.id" :to="{ name: 'full-post', params: { slug: post.slug } }"
          ...
          <div class="flex items-center gap-2 flex-nowrap overflow-x-auto sm:flex-wrap sm:overflow-visible">
            <span
              class="bg-transparent text-forest-link border border-forest-link text-[12px] sm:text-body-sm font-medium px-2.5 sm:px-3 py-0.5 rounded-circular tracking-tight font-inter whitespace-nowrap">
              {{ post.category || 'General' }}
            </span>
            <span v-for="tag in post.tags.slice(0, 3)" :key="tag"
              class="bg-transparent text-ash-gray border border-steel-border text-[12px] sm:text-body-sm px-2.5 sm:px-3 py-0.5 rounded-circular font-inter whitespace-nowrap">
              #{{ tag }}
            </span>
          </div>
          <h2
            class="font-pp-mori font-bold text-heading-sm sm:text-[32px] leading-heading-sm text-midnight-pine group-hover:text-forest-link transition-colors line-clamp-2">
            {{ post.title }}
          </h2>
          <p class="font-inter text-slate-text text-body-sm leading-[1.4] line-clamp-3">
            {{ post.content }}
          </p>
          <div class="flex items-center justify-between mt-2 pt-4 border-t border-light-gray-border">
            <div class="flex items-center gap-3">
              <div
                class="w-8 h-8 rounded-circular border border-steel-border flex items-center justify-center text-[14px] text-midnight-pine font-medium uppercase">
                {{ post.author?.charAt(0) || '?' }}
              </div>
              <span class="font-inter text-ash-gray text-body-sm">{{ post.author || 'Anonimo' }}</span>
            </div>
            <div class="flex items-center gap-2 font-inter text-ash-gray text-body-sm">
              <Calendar :size="16" />
      ...
      <div v-else-if="!loading" class="flex flex-col items-center justify-center py-24 gap-6 animate-fade-in">
        ...
        <p class="font-inter text-ash-gray text-subheading">No encontramos articulos para esa busqueda.</p>
      </div>
